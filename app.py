...
def add_bg_from_local(image_file):
    try:
        ...
        .stApp::before {{
            ...
            background-color: rgba(0, 0, 0, 0.7); /* Overlay gelap supaya tulisan nampak jelas */
            z-index: -1;
        }}
        </style>
        """,
        ...
        st.warning("⚠️ Gambar background tidak dijumpai. Pastikan nama fail betul dan berada di dalam folder yang sama.")

# TUKAR NAMA FAIL DI BAWAH KEPADA NAMA GAMBAR ANDA
...
if live_data:
    ...
    ph_error = abs(live_data["pH"] - predicted_ph) # Kira sensor drift
    ...
    # Kira tahap keyakinan model (Confidence Score)
    ...
    st.info(
        ...
        f"Sekiranya perbezaan (Drift) melebihi paras normal, sensor mungkin perlu diperiksa. "
    ...
    with f_col2:
        ...
        # Tukar warna background graf supaya ngam dengan tema gelap Streamlit
        fig.patch.set_facecolor('#0e1117') 
        ax.set_facecolor('#0e1117')
        
        ax.barh(feature_importance_df["Feature"], feature_importance_df["Importance Score"], color='skyblue')
        
        # Tukar warna tulisan graf kepada putih
        ...
        # Tambah grid yang samar untuk nampak lebih profesional
        ax.grid(axis='x', color='gray', linestyle='--', alpha=0.5)
        
        st.pyplot(fig)

# Link back to the original Datacake visual dashboard
st.markdown("---")
